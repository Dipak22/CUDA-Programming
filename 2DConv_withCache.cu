...
#define WIDTH 16
#define RADIUS 2
__constant__ float F_d[(2*RADIUS+1)*(2*RADIUS+1)];
__global__ void Conv2D_Basic(float* N,float* P,int filter_width){
        ...
        for(int r = 0;r<filter_width;r++){
            for(int c=0;c<filter_width;c++){
                ...
                if(inCol>=0 && inCol<WIDTH && inRow>=0 && inRow<HEIGHT){
                    pValue +=N[inRow*WIDTH+inCol] * F_d[r*filter_width+ c];
                }
            }
        }
...
int main(){
    ...
    cudaMemcpyToSymbol(F_d,F,filter_width*filter_width*sizeof(float));
    dim3 dimBlock(16,16,1);
    dim3 dimGrid(ceil(WIDTH/16.0),ceil(HEIGHT/16.0),1);

    Conv2D_Basic<<<dimGrid,dimBlock>>>(N_d,P_d,filter_width);
